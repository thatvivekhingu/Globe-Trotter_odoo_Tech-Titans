...
export function CreateTripPage() {
  ...
  const { currentUser, dispatch, notify } = useTripWise()
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (startDate && endDate && endDate < startDate) nextErrors.endDate = 'End date cannot be before the start date.'
    ...
    dispatch({
      ...
      trip: {
        ...
        name: name.trim(),
        description: description.trim() || 'A new chapter waiting to be planned.',
        startDate,
        endDate,
        currency: 'INR',
        budgetLimit: budgetLimit ? Number(budgetLimit) : undefined,
        coverImageUrl: coverImageUrl.trim() || imageAssets.amalfi,
        status: 'draft',
        createdAt: now,
        updatedAt: now,
      },
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)]">
        <Card className="bg-white/60">
          <form className="space-y-6" onSubmit={submit} noValidate>
            ...
            <div className="grid gap-5 sm:grid-cols-2"><Field label="Working budget" htmlFor="budget-limit" hint="Optional · used for budget warnings"><TextInput id="budget-limit" type="number" min="0" value={budgetLimit} onChange={(event) => setBudgetLimit(event.target.value)} placeholder="50000" /></Field><Field label="Currency" htmlFor="currency"><Select id="currency" options={[{ value: 'INR', label: '₹ Indian Rupee (INR)' }]} /></Field></div>
            <Field label="Cover image URL" htmlFor="cover-image" hint="Optional · use a public image URL for the trip cover"><div className="relative"><Image size={17} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-ink/35" /><TextInput id="cover-image" value={coverImageUrl} onChange={(event) => setCoverImageUrl(event.target.value)} className="pl-10" /></div></Field>
            <div className="flex flex-col-reverse gap-3 border-t border-line pt-5 sm:flex-row sm:justify-end"><Button type="button" variant="ghost" onClick={() => navigate(-1)}>Cancel</Button><Button type="submit" icon={<Sparkles size={16} />}>Create trip</Button></div>
          </form>
        </Card>
        <Card padding="none" className="overflow-hidden bg-ink text-parchment">
          <div className="relative aspect-[1.2/1] overflow-hidden"><img src={coverImageUrl || imageAssets.amalfi} alt="Trip cover preview" className="size-full object-cover opacity-75" /><div className="absolute inset-0 bg-gradient-to-t from-ink via-transparent to-transparent" /><div className="absolute bottom-5 left-5 right-5"><p className="eyebrow text-parchment/60">Your next escape</p><p className="mt-2 font-display text-3xl leading-none">{name || 'A trip worth remembering'}</p><p className="mt-3 text-xs text-parchment/65">{startDate} → {endDate}</p></div></div>
