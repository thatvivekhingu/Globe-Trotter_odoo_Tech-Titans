...
export function SharedItineraryPage() {
  ...
  return (
    <div className="min-h-screen bg-parchment">
      <header className="flex items-center justify-between gap-4 border-b border-line px-5 py-5 sm:px-10"><Link to="/dashboard" className="font-display text-2xl font-semibold tracking-[-0.04em] text-ink">TripWise<span className="text-clay">.</span></Link><div className="flex items-center gap-2"><Button variant="secondary" size="sm" icon={copied ? <Check size={15} /> : <Copy size={15} />} onClick={copyLink}>{copied ? 'Link copied' : 'Copy link'}</Button><Button size="sm" icon={<Share2 size={15} />} onClick={copyTrip}>Copy trip</Button></div></header>
