...
export function BuilderDetailPanel({ city, selectedActivity, selectedTripActivity, onUpdateActivity }: BuilderDetailPanelProps) {
  return (
    <Card padding="none" className="sticky top-24 overflow-hidden bg-white/58">
      ...
      <div className="p-5 sm:p-6">{selectedActivity && selectedTripActivity ? <><p className="eyebrow">Selected anchor</p><h2 className="mt-2 font-display text-3xl font-medium leading-none tracking-[-0.04em] text-ink">{selectedActivity.name}</h2><p className="body-copy mt-3 text-sm">{selectedActivity.description}</p><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-1"><Field label="Date" htmlFor="selected-date"><TextInput id="selected-date" type="date" value={selectedTripActivity.date} onChange={(event) => onUpdateActivity({ date: event.target.value })} /></Field><Field label="Start time" htmlFor="selected-time"><TextInput id="selected-time" type="time" value={selectedTripActivity.startTime} onChange={(event) => onUpdateActivity({ startTime: event.target.value })} /></Field><Field label="Estimated cost" htmlFor="selected-cost"><TextInput id="selected-cost" type="number" min="0" value={selectedTripActivity.estimatedCost} onChange={(event) => onUpdateActivity({ estimatedCost: Number(event.target.value) || 0 })} /></Field></div></> : <div className="py-6 text-center"><BedDouble size={24} className="mx-auto text-clay" /><h2 className="mt-3 font-display text-2xl font-medium text-ink">Choose a day or activity</h2><p className="body-copy mt-2 text-sm">Your selected details will appear here for a quick edit.</p></div>}</div>
    </Card>
  )
}
